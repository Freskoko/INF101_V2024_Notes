...
public class Inf101Graphics {
  ...
  /**
   * Draw a string centered in the given rectangle. The string will be vertically centered and
   * horizontally centered with respect to the given rectangle. If the string is too wide or to tall
   * to fit in the rectangle, it will overflow. If the width and the height are 0, the string will
   * be centered around the x and y coordinates.
   *
   * @param g The graphics context to draw on
   * @param s The string to draw
  ...
  public static void drawCenteredString(
      Graphics g, String s, double x, double y, double width, double height) {
    drawCenteredString(g, s, x + width / 2, y + height / 2);
  }

  /**
   * Draw a string centered in the given shape. The string will be vertically centered and
   * horizontally centered with respect to the bounding box of the shape. If the string is too wide
   * or to tall to fit within the bounds, it will overflow.
   *
   * @param g The graphics context to draw on
   * @param s The string to draw
  ...
  public static void drawCenteredString(Graphics g, String s, Shape shape) {
    Inf101Graphics.drawCenteredString(
        g, s, shape.getBounds2D().getCenterX(), shape.getBounds2D().getCenterY());
  }
  ...
  public static void drawImage(
      Graphics g, Image image, double x, double y, double scale, double radians) {
    double imageWidth = image.getWidth(null);
    double imageHeight = image.getHeight(null);
    Dimension2D newSize = getImageSize(image, scale, radians);
    AffineTransform transform = new AffineTransform();
    transform.translate(
        x + newSize.getWidth() / 2, y + newSize.getHeight() / 2); // last (not first!)
  ...
  public static void drawCenteredImage(
      Graphics g, Image image, double cx, double cy, double scale, double radians) {
    Dimension2D newSize = getImageSize(image, scale, radians);
    drawImage(
        g, image, cx - (newSize.getWidth()) / 2, cy - (newSize.getHeight()) / 2, scale, radians);
  }
   ...
   */
  public static void drawCenteredImage(
      Graphics g, Image image, double cx, double cy, double scale) {
    Inf101Graphics.drawCenteredImage(g, image, cx, cy, scale, 0);
  }

  /**
   * Get the size of the (bounding box of the) image after it has been scaled and rotated
   *
   * @param image The image
   * @param scale The scale factor (1.0 is 100%)
   * @param radians The angle to rotate the image in radians
   * @return The size of the (bounding box of) the image after it has been scaled and rotated
   */
  ...
  /**
   * Converts an image file into a BufferedImage. Note that in a maven project, the requested image
   * file must be in the src/main/resources folder. It accepts most common image formats, such as
   * jpg, png, etc.
   *
